...
func TestClientCall(t *testing.T) {
	tests := []struct {
		Buf     *bytes.Buffer
		Calls   []w3protypes.Caller
		WantErr error
	}{
		{
			Buf:   bytes.NewBufferString(jsonCalls1),
			Calls: []w3protypes.Caller{&testCaller{}},
		},
		{
			Buf:     bytes.NewBufferString(jsonCalls1),
			Calls:   []w3protypes.Caller{&testCaller{RequestErr: errors.New("err")}},
			WantErr: errors.New("err"),
		},
		{
			Buf:     bytes.NewBufferString(jsonCalls1),
			Calls:   []w3protypes.Caller{&testCaller{ReturnErr: errors.New("err")}},
...
func BenchmarkCall_Balance100(b *testing.B) {
	...
	b.Run("Batch", func(b *testing.B) {
		var balance big.Int
		for i := 0; i < b.N; i++ {
			requests := make([]w3protypes.Caller, len(addr100))
...
func BenchmarkCall_Block100(b *testing.B) {
	...
	b.Run("Batch", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			requests := make([]w3protypes.Caller, len(block100))
			for j := 0; j < len(requests); j++ {
